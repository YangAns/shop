...
<head>
    ...
    <%-- 添加layui-dtree样式 --%>
    <link rel="stylesheet" href="${ctx}/statics/layui_ext/dtree/dtree.css">
    <link rel="stylesheet" href="${ctx}/statics/layui_ext/dtree/font/dtreefont.css">
</head>
<body>
...
<script>
    layui.extend({
        dtree:"${ctx}/statics/layui_ext/dtree/dtree",
        ...
        function deleteById(data) {
            $.get("${ctx}/admin/role/checkRoleHaveAdmin",{"id":data.id},function (result) {
                if(result.exist){
                    layer.msg(result.message,{icon:5});
                }else{
                    //提示用户是否删除
                    layer.confirm("确定要删除[<font color='red'>" + data.roleName + "</font>]吗",
                        {icon: 3, title: '提示'}, function (index) {
                            //发送ajax请求进行删除
                            $.post("${ctx}/admin/role/deleteById", {"id": data.id}, function (result) {
                                if (result.success) {
            ...
            mainIndex = layer.open({
                ...
                ,yes: function(index, layero){
                    ...
                    if(params.length>0){
                        ...
                        //发送ajax请求，保存选中的菜单ID
                        $.post("${ctx}/admin/role/saveRoleMenu",{"ids":ids,"roleId":data.id},function(result){
                            layer.msg(result.message,{icon:1});
                        },"json");
                    }else{
                        layer.msg("请选择要分配的菜单");
                ...
                success: function () {
                    ...
                    dtree.render({
                        ...
                        url: "${ctx}/admin/role/initMenuTree?roleId="+data.id, //请求地址
